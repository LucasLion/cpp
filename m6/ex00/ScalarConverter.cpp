...
/*   Updated: 2023/09/05 15:23:26 by llion@student    ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ScalarConverter::convertFromInt( const std::string& str ) {
	...
	//iss >> i;
	std::cout << Y << i - static_cast<int>(i) << RE << std::endl;
	if (isprint(i))
...
void	ScalarConverter::convertFromDouble( const std::string& str ) {
	...
	if (i - static_cast<int>(i) != 0) {
		std::cout << "float: " << std::fixed << std::setprecision(1) << static_cast<float>(i) << "f" << std::endl;
		std::cout << "double: " << std::fixed << std::setprecision(1) << static_cast<double>(i) << std::defaultfloat << std::endl;
	}
	else {
		std::cout << "float: " << static_cast<float>(i) << ".0f" << std::endl;
...
void	ScalarConverter::convert( const std::string& str ) {
	...
	if (result == "int")
		...
		this->convertFromFloat(str);
	else if (result == "double")
		this->convertFromDouble(str);
	else
